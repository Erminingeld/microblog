...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Ваш пост размещен!'))
        # Избегает вставки повторяющихся сообщений 
...
# Показывает глобальный поток сообщений
@bp.route('/explore')
@login_required
def explore():
    page = request.args.get('page', 1, type=int)
...
# Функция просмотра страницы пользвателя
...
# Функция просмотра изменения данных пользователя
...
# Реализация подписки на пользователя
...
# Реализация отписки от пользователя
